 ...
 */
public class RecycleViewDiscoverCategory extends RecyclerView.Adapter<RecycleViewDiscoverCategory.MyViewHolder> {
    ...
    public RecycleViewDiscoverCategory(ArrayList<TicketInfo> data) {
        this.data = data;
        imageLoader = ImageLoader.getInstance();
    }

    @Override
    public RecycleViewDiscoverCategory.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_item_offers, parent, false);

        return new RecycleViewDiscoverCategory.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final RecycleViewDiscoverCategory.MyViewHolder holder, final int position) {
                ...
                if (data.get(position).getMy_likes_contribution().size() > 0) {
                    LikeTicketAsyncTask likeTicketAsyncTask = new LikeTicketAsyncTask(context, new StringResultCallback() {
                        ...
                        public void processFinish(String result, KProgressHUD kProgressHUD) {
                            ...
                            intent.putExtra("ticket_id", data.get(position).getId());
